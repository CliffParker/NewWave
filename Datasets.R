# Mon Mar  6 08:26:01 2017 ------------------------------
#' The Data available is from A.A.King on cities from England and Wales.
#' The availability is
...
#' load(datfile) 
#' 
#' Measles Cases from 1944 to 1965 for all England and Wales by cities.
#' Birth and Pop Cases   from 1940 to 1964* for all England and Wales by cities.
#' The datasets are inthe coord, demog, and measles objects.
#' Data timeline is 1944-1964
library(pomp)
library(plyr)
library(reshape2)
library(magrittr)
...
# Fri Mar 10 21:00:19 2017 ------------------------------

# local({
#   setwd("/Users/cliffordallotey/Data")
#   for (names in levels(demog$town)) {
#     assign(paste0(names,"_cases.rds"),subset(demog, town == names))
#     saveRDS(paste0(names,"_cases.rds"), file= file.path("/Users/cliffordallotey/Data", paste0(names,"_cases.rds")))
#     readRDS(paste0(names,"_cases.rds"))
#   }
#   setwd("/Users/cliffordallotey")
# })

# setwd("/Users/cliffordallotey/Data")
# for (names in levels(demog$town)) {
#   assign(paste0(names,"_cases.rda"),subset(demog, town == names))
#   save(list=paste0(names,"_cases.rda"), file= file.path("/Users/cliffordallotey/Data", paste0(names,"_cases.rda")))
#   load(paste0(names,"_cases.rda"))
# }
# setwd("/Users/cliffordallotey")
# Tue Mar  7 09:09:29 2017 ------------------------------
#'comments about the above functions
#'the save and load works great for multiple files but will load file only into names it was saved in, where as save 
#' and readRDS can let you load into a different name. 
#' 
#----

# Fri Mar 10 21:02:27 2017 ------------------------------
